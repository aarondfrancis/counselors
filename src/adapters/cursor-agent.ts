...
export class CursorAgentAdapter extends BaseAdapter {
  id = 'cursor-agent';
  displayName = 'Cursor Agent';
  commands = ['cursor-agent']; // while Cursor CLI also can be called via `agent`, this is a bit more clear
  ...
  models = [
    {
      ...
      compoundId: 'cursor-composer-1.5',
      name: 'Composer 1.5',
  ...
  buildInvocation(req: RunRequest): Invocation {
    ...
    return { cmd: req.binary ?? 'cursor-agent', args, cwd: req.cwd };
  }
}
